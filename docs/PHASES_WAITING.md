...
| H-07 | **ThreadBot 실제 자동화 워커** | 운영 | Render Background Worker 또는 Meta API 워커 배포 (현재 UI는 `last_run_at` 기록 + SKIP 조기중단만) |
...
| H-09 | **SSE 타임라인 인증 UX** | 개발 | `/api/timeline/stream?store=&token=` — 클라이언트에서 EventSource + 세션 토큰 연동 (현재 4초 폴링으로 동작) |
| H-10 | **Neon Auth trusted origins** | 운영 | 프리뷰/커스텀 도메인 추가 시 Neon Console → Auth → Trusted Origins 갱신 |

## 🟢 검증 체크리스트 (배포 후)

- [ ] 데모 로그인 → Admin 통합 타임라인 표시
- [ ] 블로그 원고 생성 → `verificationPassed: true` 시 completed
- [ ] 소셜 즉시 발행 → `publishing` → `published` (중복 클릭 시 claim 거부)
- [ ] Cron social-publish → 예약글 SKIP LOCKED claim 후 발행
- [ ] ThreadBot 댓글 생성 → 광고성 게시글 SKIP 조기중단
